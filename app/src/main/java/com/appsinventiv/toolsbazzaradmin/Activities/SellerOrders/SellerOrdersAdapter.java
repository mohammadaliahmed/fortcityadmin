...
import com.appsinventiv.toolsbazzaradmin.Activities.Orders.ViewDeliveryShippedOrder;
...
public class SellerOrdersAdapter extends RecyclerView.Adapter<SellerOrdersAdapter.ViewHolder> {
    ...
    public SellerOrdersAdapter(Context context, ArrayList<OrderModel> itemList, UpdateOrderStatus updateOrderStatus) {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.seller_order_item_layout, parent, false);
        SellerOrdersAdapter.ViewHolder viewHolder = new SellerOrdersAdapter.ViewHolder(view);
        return viewHolder;
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        holder.sellerName.setText("Seller: " + model.getCountModelArrayList().get(0).getProduct().getVendor().getStoreName());
...
//            holder.address.setText(model.getCustomer().getAddress() + ", " + model.getCustomer().getCity() + ", " + model.getCustomer().getCountry());
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        TextView orderTime, sellerName, customerName, address, orderNumber, orderItems, orderAmount, paymentMethod, orderStatus, orderTracking, customerType;
